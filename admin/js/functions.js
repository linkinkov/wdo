...
$('#warn-user-modal').on('show.bs.modal', function(e){
	...
	app.im.getDialogId(recipient_id,function(response){
		...
		{
			$(submit_btn).data('dialog_id',response.dialog_id).attr('dialog_id',response.dialog_id);
			$(submit_btn).data('recipient_id',recipient_id).attr('recipient_id',recipient_id);
			$(submit_btn).data('block_type',block_type).attr('block_type',block_type);
